...
可以通过配置 ~denote-file-type~ 设定笔记文件的类型，默认是org，或者在 ~denote-prompts~ 中增加 ~file-type~ ，这样在每次创建笔记时都可以单独指定类型。

如果想要对笔记进行分类分目录管理，在 ~denote-prompts~ 中增加 ~subdirectory~ ，创建笔记的时候会提示进行目录的选择，包含 ~denote-directory~ 目录和它所有的子目录，子目录需要自己提前创建，Denote不会自己创建子目录。
