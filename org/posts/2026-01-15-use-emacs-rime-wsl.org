...
#+AUTHOR: Chuck
#+DESCRIPTION: 在WSL中使用Emacs时，无法直接使用Windows安装的RIME输入法，当前比较好的方案是在WSL中安装librime，在Emacs中使用emacs-rime作为前端，使用体验比较贴近原生RIME输入法。
#+KEYWORDS: Emacs, WSL, RIME
#+DATE: <2026-01-15 Thu 13:19>

在WSL中使用Emacs时，无法直接使用Windows安装的RIME输入法，当前比较好的方案是在WSL中安装[[https://github.com/rime/librime][librime]]，在Emacs中使用[[https://github.com/DogLooksGood/emacs-rime][emacs-rime]]作为前端，使用体验比较贴近原生RIME输入法。
