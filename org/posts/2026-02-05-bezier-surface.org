#+TITLE: 贝塞尔曲面
#+AUTHOR: Chuck
#+DESCRIPTION: 贝塞尔曲面是贝塞尔曲线向二维参数空间的扩展，通常通过两个方向的张量积（Tensor Product）定义。
#+KEYWORDS: Computer Vision
#+DATE: <2026-02-05 Thu 08:55>

贝塞尔曲面是[[file:2026-02-04-bezier-curve.org][贝塞尔曲线]]向二维参数空间的扩展，通常通过两个方向的张量积（Tensor Product）定义。

#+caption: Sample Bézier surface; red – control points, blue – control grid, black – surface approximation
[[https://upload.wikimedia.org/wikipedia/commons/b/bf/B%C3%A9zier_surface_example.svg]]
...
- **$\mathbf{M}_n, \mathbf{M}_m$**: 分别为 $n$ 阶和 $m$ 阶的 **Bézier 基矩阵** （即[[file:2026-02-04-bezier-curve.org][前面推导过的上三角矩阵]]）。
